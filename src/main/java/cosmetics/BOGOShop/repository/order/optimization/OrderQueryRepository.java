...
public class OrderQueryRepository {
    ...
    private List<OrderItemQueryDto> findOrderItems(Long orderId){
        return em.createQuery("select new cosmetics.BOGOShop.repository.order.optimization.OrderItemQueryDto(oi.order.id, i.name,oi.orderPrice, oi.count)" +
                " from OrderItem oi" +
                " join oi.item i" +
    ...
    private Map<Long, List<OrderItemQueryDto>> findOrderItemMap(List<Long> ordersIds) {
        List<OrderItemQueryDto> orderItems = em.createQuery(
                        "select new cosmetics.BOGOShop.repository.order.optimization.OrderItemQueryDto(oi.order.id, i.name, oi.orderPrice, oi.count)" +
                                " from OrderItem oi" +
                                " join oi.item i" +
